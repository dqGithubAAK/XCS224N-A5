...
class NameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print(f"Index: {idx}, x type: {type(x)}, y type: {type(y)}, x: {x}, y: {y}")
        return x, y
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_document = document[:random.randint(4,max_length)]
        masked_length = random.randint(1, max(1, len(truncated_document) // 2))
        end_idx_prefix = random.randint(0, len(truncated_document) - masked_length)
        start_idx_suffix = end_idx_prefix + masked_length
        prefix_doc = truncated_document[:end_idx_prefix]
        masked_doc = truncated_document[end_idx_prefix:start_idx_suffix]
        suffix_doc = truncated_document[start_idx_suffix:]
        masked_string = (
        prefix_doc +
        self.MASK_CHAR +
        suffix_doc +
