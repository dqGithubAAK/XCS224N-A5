...
class TrainerConfig:
    ...
    max_epochs = 10
    batch_size = 32  # Adjusted batch size for word-level training (may need further tuning)
    learning_rate = 3e-4
    betas = (0.9, 0.95)
    grad_norm_clip = 1.0
    ...
    lr_decay = False
    warmup_tokens = 3.75e6  # Reduced warmup_tokens since we're working at word-level (may need further tuning)
    final_tokens = 26e9 # Adjusted token count for the larger context size and word-level (may need further tuning)
    # checkpoint settings
    ckpt_path = None
        ...
        def run_epoch(split):
            ...
            for it, (x, y) in pbar:
                ...
                if is_train:
                    ...
                    if config.lr_decay:
                        self.tokens += (y != config.PAD_TOKEN).sum()  # Count non-padding tokens
