...
class Block(nn.Module):
    ...
    def forward(self, x, padding_mask=None):
        attn_output = self.attn(self.ln1(x), padding_mask)
        x = x + attn_output
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, padding_mask=None):
        ...
        if targets is not None:
            # Mask the loss computation for padding tokens
            active_loss = targets != self.padding_idx
            active_logits = logits.view(-1, logits.size(-1))[active_loss.view(-1)]
            active_labels = targets.view(-1)[active_loss.view(-1)]
...
class DownProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        ### START CODE HERE
        super().__init__()
        self.C =  nn.Parameter(torch.empty(1, config.bottleneck_dim, config.n_embd))  # torch.Parameter(torch.empty())...
    ...
    def forward(self, x_input):
        ...
        ### START CODE HERE
        attn_output = self.attn(self.ln1(x_input), self.C)
        x = self.C + attn_output
        x = x + self.mlp(self.ln2(x))
        return x
        ### END CODE HERE
